...
export function login(email, password, navigate){
  return async (dispatch) => {
     ...
     try {
      ...
      const userImage = response.data?.existinggUser?.image
        ? response.data.existinggUser.image
        
        : `https://api.dicebear.com/8.x/initials/svg?seed=${response.data.existinggUser.firstName} ${response.data.existinggUser.lastName}`
      dispatch(setUser({ ...response.data.existinggUser, image: userImage }))
      localStorage.setItem("token", JSON.stringify(response.data.token))
      localStorage.setItem("user", JSON.stringify(response.data.existinggUser))
